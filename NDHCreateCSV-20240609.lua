...
local function processphotos(csvfile, timemode, defaultoffset, range)
  ...
  for pass = 1, 1 do
	...
	elseif (clearnotmatched and keywordnames ~= nil and string.match(keywordnames, "CaptionFromStrava")) then
      ...
      if ((newcaption == nil or newcaption == "!") and NDHgps.capstart ~= nil and NDHgps.capstart <= math.max(timelocal, timeutc) and NDHgps.capend ~= nil and NDHgps.capend >= math.min(timelocal, timeutc)) then
	...
	if (count == 0) then
	  file:write("Pathname,DateTimeOriginal,DateTime,Offset,OffsetLast,OffsetFromGPSMatch,TimeSource,Exception,OffsetSource,TimeLocal,TimeUTC,Camera,Status,OldLat,OldLong,NewLat,NewLong,Dist,Lat,Long,Flag,Stars,OldTitle,Title,OldCaption,TrackName,Caption,OldKeywords,Keywords,Collections,INatID,INatPhotoID,INatPhoto\n")
	end

	--                           pa   da da of of of   ts     ex     os   ti ti ca st ol ol nl nl di la lo fl st   oldti  nwtit  oldca  trknm  cap    oldke  key    coll ID PID  Photo
	local txt = string.format("\"%s\",%s,%s,%s,%s,%s,\"%s\",\"%s\",\"%s\",%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",%s,%s,\"%s\"\n",
				     csvprotect(path),
					  datetimeoriginalstr,
					     datetimestr,
						offset or "",
													    ...
													    lon,
														  ...
														  stars,
														       csvprotect(title),
															      csvprotect(newtitle),
																     csvprotect(caption),
																	  csvprotect((trk and trk['trkname']) or (cap and cap['Activity Name']) or ""),
